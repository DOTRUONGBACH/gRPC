...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withAdmin; query != nil {
		if err := cq.loadAdmin(ctx, query, nodes, nil,
			func(n *Category, e *User) { n.Edges.Admin = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadAdmin(ctx context.Context, query *UserQuery, nodes []*Category, init func(*Category), assign func(*Category, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Category)
	for i := range nodes {
		if nodes[i].category_admin == nil {
			continue
		}
		fk := *nodes[i].category_admin
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_admin" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
