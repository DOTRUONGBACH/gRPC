...
func (*Category) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case category.ForeignKeys[0]: // category_admin
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case category.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field category_admin", value)
			} else if value.Valid {
				c.category_admin = new(int)
				*c.category_admin = int(value.Int64)
			}
		default:
...
func (c *Category) QueryAdmin() *UserQuery {
	return NewCategoryClient(c.config).QueryAdmin(c)
}
